...
import numpy as np
import pandas as pd
from .base_binary_dataset import BaseDataset
import sys
sys.path.append("..")
...
class ADULT_BINARY(BaseDataset):
    ...
    def __init__(self, name='ADULT', single_bit_binary=False, device='cpu', random_state=42):
        ...
        train_data_df = pd.read_csv('datasets/ADULT/adult.data', delimiter=', ', names=list(self.features.keys()), usecols=[0, 1, 2, 3, 4, 9, 10, 11, 12, 14], engine='python')
        test_data_df = pd.read_csv('datasets/ADULT/adult.test', delimiter=', ', names=list(self.features.keys()), usecols=[0, 1, 2, 3, 4, 9, 10, 11, 12, 14], skiprows=1, engine='python')
